...
void testFittingSteps()
{
    ...
    // «агружаем координаты положений маркеров в пространстве
    ...
    // «адаем размеры частей тела и точки креплени€ маркеров (метры)
    dims.sizeAF = 0.40f;
    dims.sizeAB = 0.27f;
    dims.sizeBC = 0.27f;
    ...
    // ћасштабируем координаты маркеров под размеры модели
    scale = model.EvaluateScale(markerPts);
    markerPts.ScaleUp(scale);

    // »нициализируем контекст визуализации
    ...
    // »нициализируем процесс совмещени€ модели с маркерами
    ...
    // ѕроводим цикл пошагового совмещени€
    int istep = 0;
    bool finish;
    do
    {
...
// “ест визуализации дл€ модели, заданной углами ориентации элементов в
...
void testHumanModelAbs()
{
	...
	// »нициализируем состо€ние модели
	Sensor sensor;
